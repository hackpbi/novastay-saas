...
function FmtPickupRevenue({ n, fontColor, unit = '백만원' }: { n: number; fontColor?: string; unit?: '원' | '천원' | '백만원' }) {
  ...
  const sign  = n > 0 ? '+' : ''
  const color = n > 0 ? (fontColor ?? 'var(--color-text-primary)') : 'var(--color-negative)'
  const text  = unit === '백만원' ? Math.round(n / 1_000_000).toLocaleString()
...
export default function MonthlyPickupSegTotalModal({
  ...
  const fmtRev = (val: number) => {
    if (val === 0) return '—'
    const sign = val > 0 ? '+' : ''
    if (revUnit === '백만원') return `${sign}${Math.round(val / 1_000_000).toLocaleString()}`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4" style={{ paddingTop: 80 }}>
      ...
      <div
        ...
        style={{ width: 1100, maxWidth: '92vw', maxHeight: '79vh', background: '#000000', border: '0.5px solid rgba(0,229,160,0.2)', borderLeft: '1.5px solid #00E5A0', borderRadius: 10, overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}
      >
        {/* Header */}
