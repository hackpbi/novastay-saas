    ...
    <div className="space-y-2">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          ...
          style={{ height: 36, background: 'var(--color-bg-tertiary)' }}
        />
      ))}
    </div>
  )
...
function DataTable({ rows, summary }: { rows: SegTableRow[]; summary: SegTableSummary }) {
      ...
      <table className="w-full text-sm" style={{ borderCollapse: 'collapse' }}>
        <thead style={{ position: 'sticky', top: 0, zIndex: 1 }}>
          <tr>
            ...
            <th
              colSpan={3}
              style={{ ...thBase, textAlign: 'center', borderLeft: BORDER_GROUP }}
            >
              현재 OTB
            </th>
            <th
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {loading ? (
            ...
            <p className="text-center py-12 text-sm" style={{ color: 'var(--brand-dimmed)' }}>
              이 기간에 표시할 segmentation 데이터가 없습니다.
            </p>
          ) : (
            <DataTable rows={rows} summary={summary} />
          )}
        </div>
