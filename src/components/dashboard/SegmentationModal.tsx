...
function FmtRev({ n, fontColor, unit = '백만원' }: { n: number; fontColor?: string; unit?: '원' | '천원' | '백만원' }) {
  return n === 0 ? <Dash fontColor={fontColor} /> : <span style={{ color: fontColor }}>{unit === '백만원' ? Math.round(n / 1_000_000).toLocaleString() : unit === '천원' ? Math.round(n / 1000).toLocaleString() : Math.round(n).toLocaleString()}</span>
}
...
function DeltaRev({ v, fontColor, unit = '백만원' }: { v: number; fontColor?: string; unit?: '원' | '천원' | '백만원' }) {
  ...
  const text = unit === '백만원' ? Math.round(m).toLocaleString() : unit === '천원' ? Math.round(v / 1000).toLocaleString() : Math.round(v).toLocaleString()
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ...
        style={{ maxHeight: '88vh', background: '#000000', border: '0.5px solid rgba(0,229,160,0.2)', borderLeft: '1.5px solid #00E5A0', borderRadius: 10, overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}
      >
        {/* Header */}
