...
// 색상 — 중립(배경/글자/보더)은 테마 CSS 변수, 액센트/배지색은 원본 유지
const C = {
  pageBg:       'var(--color-bg-primary)',
...
export default function GMDailyReportPage() {
  // ── DashboardPage와 동일한 방식으로 데이터 소스 구성 (모달 props 대체) ──
  const { currentHotel } = useHotel()
  const hotelId = currentHotel?.id ?? ''
  const { otbDate, otbDates } = useDateContext()
  const { fcstDate } = useFcstDateContext()
  const [lyMode] = useState<LyPacingMode>('v1')
  const { data: lyPacingData, loading: lyLoading } = useLyPacing(lyMode)
  ...
  const forecastRows = (useForecastMonthly({ hotelId: hotelId || undefined, year: cardYear, updateDate: fcstDate || null }).data ?? []) as ForecastMonthlyRow[]
  const budgetRows = (useBudgetMonthly({ hotelId: hotelId || undefined, year: cardYear, updateDate: budgetDate }).data ?? []) as BudgetMonthlyRow[]
  const open = true
  ...
  // 인쇄 스타일 동적 주입 (페이지용 — 앱 크롬 숨김 + 리포트 페이지네이션 유지)
  useEffect(() => {
    const style = document.createElement('style')
    style.id = 'daily-report-print-style'
    style.textContent = `
      @media print {
        ...
        .gm-a4 { background: #ffffff !important; color: #000000 !important; box-shadow: none !important; border-radius: 0 !important; width: 100% !important; max-width: none !important; }
        .gm-a4 * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      }
    `
    document.head.appendChild(style)
    return () => { document.getElementById('daily-report-print-style')?.remove() }
  }, [])
      ...
      <div key={seg} style={{ background: C.cardBg, borderRadius: 8, padding: '10px 14px' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, tableLayout: 'fixed' }}>
          ...
          <tbody>
            ...
            <tr style={{ background: 'var(--color-bg-tertiary)', borderTop: '0.5px solid #c8c7c0' }}>
  ...
  const a4: React.CSSProperties = {
    width: '100%', background: C.pageBg, borderRadius: 8,
    padding: '31px 35px', position: 'relative', flexShrink: 0,
  }
  const sectionTitle: React.CSSProperties = { fontSize: 13, fontWeight: 600, color: TXT, margin: '0 0 9px' }
  const card: React.CSSProperties = { background: BG_ELEV, borderRadius: 6, padding: '13px 15px' }
  const th: React.CSSProperties = { fontSize: 10, color: TXT3, fontWeight: 400, padding: '3px 7px' }

  const report = (
      <div className="gm-a4 gm-report-content" style={a4}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', fontSize: 9 }}>
                ...
                <tbody>
                    ...
                    {chartKpi.map(d => (
                      <td key={d.date} style={{ verticalAlign: 'bottom', padding: '0 2px', height: 120 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
                          <div style={{ width: '40%', height: 'calc(100% - 20px)', background: d.isToday ? 'var(--color-bg-elevated)' : d.isPast ? 'var(--color-bg-tertiary)' : C.border, borderRadius: 4, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', position: 'relative', overflow: 'visible' }}>
                  ...
                  <tr style={{ borderTop: `0.5px solid ${C.border}` }}>
                    ...
                    {chartKpi.map(d => (
                      <td key={d.date} style={{ textAlign: 'center', padding: '3px 2px', fontSize: 9, color: C.textSecondary, fontWeight: d.isToday ? 500 : 400, background: d.isToday ? 'var(--color-bg-elevated)' : d.isPast ? 'var(--color-bg-secondary)' : 'transparent', borderBottom: `0.5px solid ${C.border}` }}>{d.adr > 0 ? <FmtVal val={`${d.adr}k`} numSize={9} /> : '-'}</td>
                    ))}
                  </tr>
                  {/* BAR 행 */}
                  <tr>
                    ...
                    {chartKpi.map(d => (
                      <td key={d.date} style={{ textAlign: 'center', padding: '3px 2px', fontSize: 9, color: C.textPrimary, fontWeight: 500, background: d.isToday ? 'var(--color-bg-elevated)' : d.isPast ? 'var(--color-bg-secondary)' : 'transparent', borderBottom: `0.5px solid ${C.border}` }}>{d.barRate !== null ? <FmtVal val={`${d.barRate}k`} numSize={9} /> : '-'}</td>
                    ))}
                  </tr>
                  {/* LY 행 */}
                  <tr>
                    ...
                    {chartKpi.map(d => (
                      <td key={d.date} style={{ textAlign: 'center', padding: '3px 2px', fontSize: 9, color: C.textMuted, background: d.isToday ? 'var(--color-bg-elevated)' : d.isPast ? 'var(--color-bg-secondary)' : 'transparent' }}>{d.lyOcc !== null ? `${d.lyOcc}%` : '-'}</td>
                    ))}
                  </tr>
                </tbody>
              </table>
  ...
  return (
    <div style={{ width: '100%', minHeight: '100vh', background: 'var(--color-bg-primary)', color: 'var(--color-text-primary)', fontFamily: 'inherit', padding: '32px 40px' }}>
      {report}
    </div>
  )
}
